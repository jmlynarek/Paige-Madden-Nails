...
- **Card payments (Stripe, added 2026-07-25):** hosted Stripe Checkout as an ADDITIVE option next to the P2P handles (which are untouched). Flow: the order-submitted screen shows a "Pay by card" tile (only when enabled) → `create-checkout-session` edge fn computes the amount due server-side (set + fees − gift credit; never trusts the client) and redirects to Stripe's hosted page (cards + Apple Pay + Cash App Pay) → Stripe calls the `stripe-webhook` edge fn (HMAC-verified, fails closed, idempotent) → `payment_collected=true / paid_at / paid_via='stripe'` + the `payment_verified` email server-side (third copy of the shared email shell — keep in sync with admin.html + send-order-confirmation). Config lives in **admin → Integrations → "Payments · Stripe"**: a guided 4-step "Set Up Stripe" card (account → `sk_` key → webhook URL + `whsec_` → toggle on + `4242…` test), steps check themselves off, badge shows test vs live. Keys in `app_settings` (`stripe_enabled`/`stripe_secret_key`/`stripe_webhook_secret`, migration `0018_stripe_checkout`) with env fallbacks `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET`. The public site learns card checkout is on via the `payment_methods` row `id='card'` (anon-readable; `saveIntegrations()` syncs its `enabled` = toggle on AND key present). Card payers can skip the PM-### memo — matching is automatic via order-id metadata. **Status: code + migration + edge fns deployed; NOT yet active — Jeremy/Paige still needs to create the Stripe account (under the Gmail) and walk the in-app setup steps.** Stripe return trip: `/?paid=1&code=PM-###` shows a "Payment received" screen (`lastOrder.paid` persists it); the redirect is cosmetic only — money is only ever marked by the webhook.
...
- **Stripe is built but not connected** — no Stripe account yet; the Integrations card's 4-step walkthrough is the setup path (test keys first, then live). Until the toggle is on, the public card tile stays hidden and both edge fns refuse politely. Refunds/disputes: handle in the Stripe dashboard (no in-app support).
