...
// Resend. Called from admin.html's notifyCustomer() when an order's
// status changes — the branded HTML is rendered client-side and passed
// in, so this function is a thin, provider-specific send step.
//
// Auth: admin-only (jeremy@idealtraits.com), same gate as buy-shipping-label.
//
// Secrets (set with `supabase secrets set ...`):
//   RESEND_API_KEY   re_...  (from https://resend.com)
//   MAIL_FROM        optional, e.g. "Paige Madden Nails <orders@paigemadden.app>"
//                    Defaults to Resend's onboarding sender (test only:
//                    delivers to the Resend account owner until you verify
//                    a sending domain).
// ============================================================
...
const MAIL_FROM =
  Deno.env.get("MAIL_FROM") || "Paige Madden Nails <onboarding@resend.dev>";
